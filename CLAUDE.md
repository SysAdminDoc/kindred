...
- **Compatibility over time**: Score history tracking with Canvas line chart
- **Profile endorsements**: Trait badges from friends/matches with counts
- **Shared interests tags**: Visual comparison (green shared, neutral unique)
- **Group post reactions**: Emoji reactions on group posts with toggle
...
- **Message editing**: Edit messages within 5-minute grace period with edit history
- **Message deletion**: Soft-delete messages (shows "This message was deleted")
- **Message status icons**: Sent/delivered/read indicators (○/✓/✓✓)
- **Clipboard image paste**: Paste images directly into chat from clipboard
- **Photo drag-reorder**: Drag-and-drop photo gallery reordering
- **Profile preview**: "See how others see you" preview overlay
- **Profile completeness**: Calculated score with actionable improvement tips
- **Retention email digest**: Weekly digest for inactive users with new likes/matches counts
- **Email digest toggle**: User setting to opt in/out of digest emails
- **Focus trap**: Keyboard trap in modals (Tab cycling, Escape to close)
...
- **Video calling**: Jitsi Meet integration with in-app call initiation, WebSocket signaling, incoming call overlay, accept/decline, call history
- **AI conversation suggestions**: Template-based + Puter.js AI fallback, clickable suggestion chips in chat, usage tracking
- **Compatibility recalculation**: Re-score all matches after questionnaire update, before/after comparison, recalc history
- **OAuth social login**: Google + Apple account linking (scaffolding), connect/disconnect UI in settings
- **Profile boost**: Premium visibility boost (1h/3h/6h), countdown timer, admin boost management
- **Events map view**: Leaflet map with CartoDB Dark Matter tiles, event pins with RSVP popups, lazy-loaded CDN
- **Public API**: Read-only API key-authenticated endpoints (profiles, events, stats), admin key management (create/revoke)
- **API key management**: Admin CRUD for API keys with SHA-256 hashing, one-time key reveal
...
- **v2.5.1** - Comprehensive audit (41 fixes). **Critical**: removed 5 `conn.close()` calls corrupting thread-local connection pool (main.py startup + send_msg + send_photo + add_friend + like_toggle, admin_app startup), fixed 3 public API SQL crashes (wrong column names: display_name→name, bio removed, user_id JOIN reversed, events name→title, likes.mutual replaced with self-join), fixed route shadowing (4 literal routes registered after wildcards: /matches/expiring, /messages/search, /profile/completeness, /profile/boost moved before /{id} routes), fixed WebSocket close-before-accept protocol violation. **High**: fixed `delete_story` missing profile_id arg (TypeError), fixed 6 admin endpoints using `/admin/` instead of `/api/admin/` prefix, fixed 5 Pydantic v2 models using `str = None` instead of `str | None = None`, fixed `body: dict` without `Body(...)` on 3 endpoints (2FA verify/disable, subscription upgrade), added auth to `DELETE /api/blog`, fixed `get_or_create_game` wrong 6th parameter, fixed `get_expiring_matches` inverted date filter (>= → <=), added 16 missing tables to GDPR `delete_account` cleanup, fixed `link_oauth_account` unhandled UNIQUE conflict, fixed `get_inactive_users` double-counting matches, fixed `export_safety_reports_csv` hardcoded 'reported' status, fixed `check_suspension_expired` re-processing same rows, fixed `react_to_story` INSERT OR REPLACE → UPSERT, added `conn.commit()` before VACUUM, fixed `bulk_delete_profiles` rollback style, fixed 3 functions with `deactivated = 0` missing NULL handling. **Frontend**: fixed `updateNotificationBadge` → `updateUnreadBadges` (undefined function breaking all WS notifications), fixed `renderMsgStatus` always showing double-check, fixed swipe-right triggering superLike instead of like, fixed `escHtml` corrupting Jitsi URLs in window.open, fixed `puter.ai.txt2txt` → `puter.ai.chat`, added null guard to `pollNotifications`, converted 56 bare `fetch()` → `authFetch()`, fixed emoji entity encoding in group reactions, fixed suggestion chip double-escaping. **Admin frontend**: added missing `--accent` CSS variable, added single-quote escaping to `escHtml()`, fixed 4 XSS points (profile badges, friend names, dealbreakers), fixed `avg_duration_ms` null crash, corrected 3 API endpoint paths. Fixed `log_audit` missing positional args (2 calls), hardened `gen_invite` to use admin ID
- **v2.5.0** - Phase 8: Premium & integrations. Video calling via Jitsi Meet (in-app call initiation, WebSocket signaling for incoming calls, accept/decline, call history). AI conversation suggestions (template-based + Puter.js fallback, clickable suggestion chips, usage tracking). Compatibility recalculation (re-score all matches after questionnaire update, before/after comparison, recalc history). OAuth social login scaffolding (Google + Apple account linking, connect/disconnect UI). Profile boost (premium visibility boost with 1h/3h/6h duration, countdown timer, admin management). Events map view (Leaflet with CartoDB Dark Matter tiles, event pins with RSVP popups, lazy-loaded CDN). Read-only public API (API key-authenticated endpoints for profiles/events/stats). Admin: API key management (create/revoke with SHA-256 hashing, one-time key reveal), AI suggestion stats, active boost management. 6 new tables (video_calls, ai_suggestions, compatibility_recalcs, oauth_accounts, profile_boosts, api_keys), 22 new CRUD functions, 21 new API endpoints
